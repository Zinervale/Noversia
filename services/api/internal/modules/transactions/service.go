...
func (s *Service) UpdateCategory(ctx context.Context, txID string, categoryID string, reason string) (Transaction, error) {
	return s.repository.UpdateTransactionCategory(ctx, txID, categoryID, reason)
}
...
func (s *Service) ImportCSV(ctx context.Context, reader io.Reader, filename string) (ImportReport, error) {
	...
	for i := range report.Rows {
		if !report.Rows[i].Valid { continue }
		if rule, ok := categorizer.Categorize(report.Rows[i].Label); ok {
			report.Rows[i].CategoryID = rule.CategoryID
			report.Rows[i].CategoryName = rule.CategoryName
			report.Rows[i].ConfidenceScore = rule.ConfidenceScore
		}
	}
