...
func ParseTransactionCSV(reader io.Reader, filename string) (ImportReport, error) {
	...
	for idx, record := range records[1:] {
		result := ImportRowResult{Line: idx + 2, Valid: true}
		...
		if dateValue == "" { result.Errors = append(result.Errors, "date obligatoire") } else if _, err := time.Parse("2006-01-02", dateValue); err != nil { result.Errors = append(result.Errors, "date invalide") } else { result.Date = dateValue }
		...
		if amountValue == "" { result.Errors = append(result.Errors, "amount obligatoire") } else if err != nil { result.Errors = append(result.Errors, "amount invalide") } else { result.Amount = amount }
		if currencyValue == "" { result.Errors = append(result.Errors, "currency obligatoire") } else { result.Currency = currencyValue }
		if len(result.Errors) > 0 { result.Valid = false; report.InvalidRows++ } else { report.ValidRows++ }
		report.Rows = append(report.Rows, result)
	}
	report.DetectedRows = len(records) - 1
	return report, nil
}
func getCSVValue(record []string, index int) string { if index < 0 || index >= len(record) { return "" }; return strings.TrimSpace(record[index]) }
