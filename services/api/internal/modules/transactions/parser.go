...
func ParseTransactionCSV(reader io.Reader, filename string) (ImportReport, error) {
	...
	for idx, record := range records[1:] {
		...
		if amountValue == "" { result.Errors = append(result.Errors, "amount obligatoire")
		} else if err != nil { result.Errors = append(result.Errors, "amount invalide")
...
func getCSVValue(record []string, index int) string {
	if index < 0 || index >= len(record) { return "" }
